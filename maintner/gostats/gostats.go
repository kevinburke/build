...
func stats(gp *maintner.GerritProject, topN int) []FileCount {
...
func main() {
	...
	if *topFiles > 0 {
		...
		var proj *maintner.GerritProject
		err := gerrit.ForeachProjectUnsorted(func(gp *maintner.GerritProject) error {
			if gp.ServerSlashProject() == "go.googlesource.com/go" {
				proj = gp
				return stop
			}
			return nil
